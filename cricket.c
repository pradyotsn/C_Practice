...
// Change the functions parameters to your comfort Samu

//Function to print details

void printDetails(char Item_name[3],
int Item_number,
char Warehouse_location[20],
char Branch_name[3],
char City[10])
{
  printf("%s \t\t %d \t\t %s \t\t %s \t\t %s \n",Item_name, Item_number, Warehouse_location, Branch_name, City);
...
// Function call to printDetails where formal parameters are struct vars  not the structs itself

printDetails(in[i].Item_name,in[i].Item_number,in[i].Warehouse_location,in[i].Branch_name,in[i].City);
}
   return 0;
}
